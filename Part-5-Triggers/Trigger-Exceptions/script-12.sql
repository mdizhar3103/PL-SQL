...
CREATE OR REPLACE TRIGGER orders_br_upd_trig
  ...
  DECLARE
    l_index NUMBER := 0;
    col_too_long EXCEPTION;
    PRAGMA EXCEPTION_INIT(col_too_long,-01438);
  BEGIN
    l_index := l_index + 1;
    test_package.g_number := test_package.g_number + 1;
    DBMS_OUTPUT.PUT_LINE('Inside before row triger to update order id: '|| :NEW.order_id||' l_index: '|| l_index|| ' test_package.g_number: '|| test_package.g_number);
  ...
  EXCEPTION
    WHEN col_too_long THEN
        RAISE_APPLICATION_ERROR(-20001,'order id length exceeds log_order_id column lenght');
    WHEN OTHERS THEN
